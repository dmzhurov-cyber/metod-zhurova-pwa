...
import { buildTrainerUserContextString, fetchLatestCohortContextBlock } from '../lib/trainerUserContext'
...
Если ниже есть блок «СВОДКА ПО КОГОРТЕ» — это анонимные агрегаты; используй только как там сказано, не выдумывай цифры. Если блока нет — не придумывай статистику «у других».`
...
export function AiTrainer() {
  ...
  const [cohortContext, setCohortContext] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    void sb.auth.getSession().then(async ({ data }) => {
      ...
      const cohort = await fetchLatestCohortContextBlock(sb)
  ...
  async function send() {
    ...
    try {
      const userContext = buildTrainerUserContextString()
      const systemParts = [SYSTEM_PROMPT, userContext, cohortContext].filter((s) => s && s.trim().length > 0)
